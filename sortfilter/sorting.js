...
const complexSort = simpleList.sort((a,b) => {
    if (a>b){
        return -1;
    } else if (a<b){
        return 1;
...
const searchList = (list, query) => {
    const searchCallback = (object, query) =>{
        return object.name.toLowerCase().includes(query.toLowerCase()) || object.description.toLowerCase().includes(query.toLowerCase()) || object.tags.some(tag => tag.toLowerCase().includes(query.toLowerCase()));
    }
    return list.filter(item => searchCallback(item, query));
}

console.log(searchList(hikes, "easy"));